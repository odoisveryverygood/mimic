...
const manifest={manifest_version:3,name:'Mimic — Your personal repeat button',version:'4.0.2',minimum_chrome_version:'125',description:'Teach everyday browser tasks once. Repeat them with new details, save time on routine clicks, and check the results.',key:identity.key,permissions:['debugger','storage','tabs','sidePanel'],host_permissions:[`${identity.dashboardOrigin}/*`],side_panel:{default_path:'panel.html'},icons:{16:'icon16.png',48:'icon48.png',128:'icon128.png'},background:{service_worker:'background.js',type:'module'},action:{default_title:'Open Mimic'},externally_connectable:{matches:origins.map(o=>`${o}/*`)}};
