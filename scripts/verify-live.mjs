...
for(const route of ['/','/studio','/practice','/extension','/privacy','/docs','/docs/product-brief.md','/docs/workflow-playbook.md','/docs/beta-validation-plan.md','/docs/reading-list.csv','/docs/trip-ideas.csv','/docs/wishlist.csv','/docs/beta-scorecard.csv','/account','/downloads/mimic-browser-companion.zip','/api/cloud/config']){
  const response=await fetch(`${base}${route}`);assert.equal(response.status,200,route);
  if(route==='/')assert.match(await response.text(),/Less clicking. More living./,'Root serves the focused task interface');
