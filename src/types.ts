export type StepType = 'navigate'|'click'|'fill'|'select'|'check'|'press'|'extract'|'manual';
...
export type Run = {id:string;commandId:string;commandName:string;mode:'test'|'run';status:string;startedAt:string;finishedAt?:string;currentStep:number;message?:string;error?:string;outputs:{label:string;text:string}[];steps:{id:string;label:string;type:StepType;status:string}[]};
export type State = {version:string;commands:Command[];demonstrations:Demo[];runs:Run[];recording:Demo|null;activeRun:Run|null;browserOpen:boolean;busy:boolean;token:string;practiceUrl:string;connected?:boolean};
